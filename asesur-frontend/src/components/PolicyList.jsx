...
export default function RecordsView() {
  ...
  // 1. URL DINÁMICA (Localhost o Nube)
  ...
  const handleSearch = async (term) => {
    ...
    setLoading(true)
    try {
      // 2. CORRECCIÓN DE ENDPOINT:
      // Cambiamos '/api/records/search' (que no existe) por '/api/clientes/search'
      // Si no hay término, traemos todos los clientes ('/clientes')
      const endpoint = term 
      ...
      // No mostramos alert si es 404 (simplemente no hay resultados)
      if (!error.message.includes('404')) {
         // alert(error.message) // Opcional
      }
      setResults([])
    } finally { 
      setLoading(false) 
  ...
  const handleDeletePolicy = async (policyId) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("No se pudo eliminar la póliza.");

      alert("🗑️ Póliza eliminada correctamente.");
      handleSearch(searchTerm); // Recargamos
  ...
  return (
    <div className="records-container">
      ...
      {loading ? <p className="loading-text">Cargando...</p> : results.length === 0 ? <p style={{textAlign:'center', color:'#64748b', padding:'20px'}}>Sin resultados.</p> : (
        <div className="results-list">
          {results.map(cliente => {
            ...
            // Seguridad: Si polizas viene null o undefined, usamos array vacío
      ...
      {/* MODAL DETALLE (Igual que antes pero con validaciones) */}
      {selectedRecord && (
                ...
                <div className="info-group">
                  ...
                  <div>
                    ...
                    {selectedRecord.cliente.ine_url ? (
                        <a href={selectedRecord.cliente.ine_url} target="_blank" rel="noopener noreferrer" className="ine-link">
                            Ver INE
                        </a>
                    ) : (
